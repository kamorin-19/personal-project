...
├── docs/
│   ├── overview-frontend.md  # フロントエンド概要キャッシュ（行数制限なし・自動更新）
│   ├── overview-backend.md   # バックエンド概要キャッシュ（行数制限なし・自動更新）
│   ├── overview-infra.md     # インフラ概要キャッシュ（行数制限なし・自動更新）
...
| エージェント | 役割 |
|---|---|
| Claude Code | overviewキャッシュ参照・設計・レビュー対応・最終判断・ユーザー対話 |
| Codex CLI | テスト設計・実装・テスト実装・修正ループ・次工程への引き継ぎ |
...
| エージェント | 使うべきケース |
|---|---|
| Claude Code | 設計・方針決定・トレードオフ判断・最終品質確認 |
| Codex CLI | テスト設計・CRUD実装・テスト追加・型修正・リファクタ・軽微バグ修正 |
| Gemini CLI | 最新仕様確認・ライブラリ比較・Web調査・ドキュメント生成・コードベース把握 |

> **設計担当について（運用メモ）**
> 現在は Claude Code が `docs/overview-*.md` を参照して設計を行う。
> トークン消費が許容範囲を超えると判断した場合は `codex-design` に設計を移管すること。
...
| `codex-impl` | 設計書をもとにした実装・テスト実装・テスト通過までの修正ループ・Claude Code への完了報告 |
| `gemini-code-overview` | コードベース全体把握（領域別に実行） |
| `gemini-code-review` | コードレビュー |
| `gemini-web-research` | Web調査・情報収集 |
| `gemini-docs` | ドキュメント作成 |

> **codex-design について（運用メモ）**
> 設計担当を Codex CLI に移管する場合は以下を復活させること。
> `codex-design`：設計書の作成・次工程（codex-test-design）への引き継ぎ指示生成
...
タスクに関係する領域のファイルのみ参照・更新すること。行数制限はない。
...
- 対象キャッシュファイルが著しく肥大化した場合（目安：内容が冗長と感じた時点）は `gemini-code-overview` を再実行して再要約・上書きすること。別ファイルへの分割は行わない
...
Claude Code が作成する設計書は以下の項目をすべて含むこと。
項目が欠けている場合、codex-test-design・codex-impl への引き渡し品質が低下するため必ず遵守すること。

**ファイル命名規則（上書き防止のため必ず守ること）：**
- 設計書: `docs/design-{yyyymmdd}-{index}.md`
...
## インターフェース定義
変更前後の関数シグネチャ・型定義・APIエンドポイントを明示
（変更がない場合は「変更なし」と記載）

## 実装方針
採用するアプローチと、採用しなかった選択肢・その理由を記載
...
**Claude Code は overviewキャッシュ参照・設計・レビュー対応を担当する。
テスト設計→実装の連鎖（手順3〜4）は各Codexエージェントが自律的に引き継ぐ。**

```
Claude Code
  │
  │ ① overviewキャッシュ確認・設計書作成
  │   （docs/overview-*.md を参照。コードベース参照は最小限に抑えること）
  ▼
Claude Code ─────────────────────────────────── 設計書を docs/ に出力
  │
  │ ② テスト設計指示（claude_to_codex/）
  ▼
...
1. **Claude Code：** タスクに関係する領域の `docs/overview-{領域}.md` を確認する。キャッシュルールに従い必要な場合のみ `gemini-code-overview` を実行して更新する。overviewキャッシュを参照して設計書を作成し `docs/` に出力する（コードベースの直接参照は最小限に抑えること）
2. **Claude Code：** 設計書のパスを添えて `codex-test-design` にテスト設計を指示する（`claude_to_codex/`）
3. **codex-test-design：** テスト設計書を作成し `docs/` に出力する。完了後、設計書・テスト設計書のパスを添えて `codex-impl` への引き継ぎ指示を `codex_to_codex/` に出力する
4. **codex-impl：** 設計書・テスト設計書をもとに実装・テスト実装を行い、全テスト通過まで修正ループを回す。完了後、完了報告を `codex_to_claude/` に出力する
5. **Claude Code：** 完了報告の STATUS と SUMMARY を確認し、`gemini-code-review` にレビューを指示する（`claude_to_gemini/`）
6. **Claude Code：** レビュー結果を受け取り、レビュー対応ルールに従い対処する。完了後 `/clear` を実行する
...
フロー1手順5に進む前に、変更内容に応じてレビューを実行するか判定すること。

| 変更の種類 | レビュー要否 |
|---|---|
| UI文言・スタイルのみの変更 | スキップ可 |
| コンポーネント追加・ロジック変更 | 実施推奨 |
| 認証・セキュリティ関連の変更 | **必須** |
...
| 指摘の種類 | 対処方法 |
|---|---|
| typo・命名・フォーマットのみ | Claude Code が直接修正する。フロー1手順5には戻らない |
| 設計変更・ロジック修正を伴う指摘 | `codex-impl` に修正を委任し、フロー1手順4から再実行する |
